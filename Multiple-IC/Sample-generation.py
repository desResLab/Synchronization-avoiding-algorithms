...
fz  	= np.random.uniform(0,1)           # external force: body force per unit area
...
newpath = r'Results/Static/cases='+str(rank) 
...
test_num  = 50000                # number of time steps
num_cri   = 9999999

# create the folder to save case-wise data
newpath = r'Results/Dynamics/cases='+str(rank) 
if not os.path.exists(newpath):
	os.makedirs(newpath)

# initialization: d0 by random sampling(from force), v0=0, a0 by solve the system once, d-1 by taylor expansion
d0           =   d_steady
...
for i in range(0,test_num):
	...
	dz = d1[2::3]
	tn = tn+dt
	savename = 'Results/Dynamics/cases='+str(rank) +'/num=' + str(i) + '.vtk'
	meshio.write_points_cells(savename, Points , Mesh.cells, {'displacement-x':dx, 'displacement-y':dy, 'displacement-z':dz})
