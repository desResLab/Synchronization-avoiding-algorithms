...
class elasticity:
	...
	def f(self,X,t):  # rhs external force using no ramping, steady version
		if self.R == False:
			return np.array([[0.0],
							 [0.0],
							 [-self.fz]])
		else:
			return np.array([[0.0],[0.0],[-self.fz*np.tanh(t)]])


# build time integration class for elementwise solver
class Time_integration:
	def __init__(self, tn, dt, d0, v0, a0, dn1):
		self.tn   = tn # current time
		self.dt   = dt # timestep size
		self.dn1  = dn1# ghost displcement step
		self.d0   = d0 # current displacement 
		self.v0   = v0 # current velocity
		self.a0   = a0 # current accleration

	def tn_plus_1(self):
		return self.tn + self.dt
	def tn_plus_half(self):
		return 0.5*(self.tn_plus_1() + self.tn)
...
# Intial displacement profile
def Initial_displacement(Points):
	d0     =   np.zeros((len(Points)*3,1)) # initialize
	# Q: what kinda of function should be used?
	dis_z  =  lambda x : 1.0/1000*(np.cos(np.pi/50*x)-1)     # lambda_function, initial displacement as a functrion of spatial points
