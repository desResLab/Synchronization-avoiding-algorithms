# This script contains functions/ classes to build the DNN model
...
class LSTM_Encoder(nn.Module):
  ...
  def forward(self,x):
    ...
    ########### ref:https://discuss.pytorch.org/t/bidirectional-3-layer-lstm-hidden-output/41336/2
...
def model_train(device, model,trainloader, criterion,  optimizer,  n_future, training_method = 'recursive', ratio = 0.5):
  ...
  for Train_batch, Truth_batch in trainloader: # batch loop
      ...
      decoder_input  = Train_batch[:,-1,:]  # last input step as decoder input
      decoder_hidden = e_hn
      decoder_cell   = e_cn
    
      # recursively decoding
      outputs = torch.zeros((Train_batch.shape[0], n_future, Train_batch.shape[2]), device=device) # init output array
      if training_method == 'recursive': # recursive decoding
        for i in range(n_future):
          decoder_output, d_hn, d_cn = model.decoder(decoder_input, decoder_hidden, decoder_cell) # forward the decoder model
          outputs[:,i,:] = decoder_output # insert prediction to the output array
          decoder_input = decoder_output  # previous output becomes input
          decoder_hidden = d_hn           # pass hidden state
          decoder_cell   = d_cn           # pass cell state
      elif training_method == 'mtf': # recursive decoding with teacher's forcing (not used in the paper)
        for i in range(n_future):
...
# model validation (test means validation here)
def model_test(device, model, testloader, criterion, n_future):
  ...
  with torch.no_grad(): # no more gradient update
...
# Do model predictions after out of the training stage
...
# scale all the values in the tensor to [-1,0] and return additional max, min values for future rescaling
# this function is used in training, X is the input, Y is the truth
...
# scale data use training max and min after out of training
def scale_forward(X, scale_max, scale_min):
  X = (X - scale_max) / (-scale_min+scale_max)
  return X

# scale data back to original scales after out of training
def scale_it_back(X,scale_max,scale_min):
  X = X*(scale_max-scale_min) + scale_max
  return X

# Training data pre-processing, cut_off by $n_ts$, sampled (filter) by $n_s$
# windowing w.r.t history and future steps (n_p, n_f)
# tensorize and send to hardware
def Dis_data_filtered_subset_coronary(device, input_size, filter_size, n_past, n_future, Path, cut_off):
  ...
  Data_numpy = Data_numpy[0:int(cut_off*len(Data_numpy)),:] # n_ts
  Data_numpy = Data_numpy[0::filter_size,:] # apply filters(n_s)
