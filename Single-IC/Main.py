...
rho 	= 1           # density
fz  	= 1           # external force: body force per unit area
...
# how frequency we are going to use the DNN model
dnn_every = 1                  # apply dnn every xx steps, 1 means dnn only   
...
num_cri   = 4000                 # before this step, no model, after that, all model
...
# using trained model, it is not reasonable to change the force, but we could change the IC only
#--by assigning quadratics
mag = 0.12343432 # some random ratio, but determistic
min_dz = min(dz) * mag

elas_d0                      = elasticity(E*nu/((1+nu)*(1-2*nu)),E/(2*(1+nu)),rho, fz/5, False) # elasticity class preparation for the steady solver
d_some_other_steady_state    = Steady_Elasticity_solver(p, Cells, Points, Dirichlet_global_dof, elas_d0, t=None, Facets=None, Neumann=None)

# initialization: 
#d0             = d_some_other_steady_state
#d0             =   IC_sampling(Points, min_dz, mag, Rnd=False) # initialize d0
